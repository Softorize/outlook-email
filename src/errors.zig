...
pub fn defaultMessage(err: Error) []const u8 {
    return switch (err) {
        ...
        error.TlsHandshakeFailed => "TLS handshake failed. If you're on a corporate network with a TLS-intercepting proxy, try --ca-bundle <path> or set OUTLOOK_CA_BUNDLE.",
        ...
        error.Unauthorized => "Sign-in expired. Run 'outlook login' again.",
        error.TokenExpiredNoRefresh => "Your saved sign-in has expired and could not be refreshed. Run 'outlook login'.",
        error.DeviceFlowDenied => "You denied the sign-in request.",
        error.DeviceFlowExpired => "The device code expired before sign-in completed. Run 'outlook login' again.",
        error.DeviceFlowPollFailed => "The sign-in poll failed. Check your network and retry 'outlook login'.",
        error.NotSignedIn => "Not signed in. Run 'outlook login' first.",
        ...
        error.NoCurrentAccount => "No account is selected. Run 'outlook login' or 'outlook use <email>'.",
        error.AccountNotFound => "That account is not signed in on this machine.",
        error.InvalidArgument => "Invalid command-line arguments. Run 'outlook --help'.",
